...
void loop() {
  ...
  if(distance <= 10){ //if the object is close
    ...
    noTone(buzzerPin); //turn the buzzer off //move the servo to 133 degrees
    delay(100); //wait 100 milliseconds
...
float getDistance(){
  float echoTime; //variable to store the time it takes for a ping to
  ...
  calcualtedDistance = echoTime / 148.0; //calculate the distance of the object that
